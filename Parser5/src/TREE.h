...
// function is called
bool BTreeNode::insertNonFull(string k)
{
	...
	if (leaf == true)
	{
		...
		//size_t myArraySize = sizeof(keys) / sizeof(int);
		
		for (int j = 0; j < n; j++) {
			if (keys[j] == k) {
				cout << "REPETIDO " << k << endl;
				return false;
				//repetido = true;
				//break;
			}
